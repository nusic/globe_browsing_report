...
A \class{Chunk Tile Pile} represents multiple \class{Chunk Tile}s across multiple mip levels. A \class{Chunk Tile Pile} contains all the information needed to perform the LOD switching described under section \todo{ref Switching}. Retrieving a \class{Chunk Tile Pile} simply requires a tile index of highest mip level and the number of desired \class{Chunk Tile}s in the pile as described in Listing \ref{lst:chunktilepile}.
...
ChunkTilePile getChunkTilePile(TileProvider* tp, TileIndex ti, int n){
...
In the example above, it can be concluded that each layer in the GLSL code must be a struct with a member called ``pile'' and a member called ``setting''. An example of a fully resolved identifier is: \\
