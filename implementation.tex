...
The most important part of the chunked LOD algorithm is the ability to dynamically select which chunks to split or merge. This is done by evaluating all the leaf nodes of the chunk tree. Chunks that are cullable will indicate that they can be merged by their chunk parent node. Chunks that cannot be culled indicate whether they want be splitted or merged based on a chunk selection algorithm. If all child nodes to a common chunk parent want to merge, the parent will merge its children. If any chunk reports it want to be splitted, its chunk node will initialize four new children within the chunk tree.
