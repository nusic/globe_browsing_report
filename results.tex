...
\begin{table}[h]
  \centering
  \caption[]{Settings for evaluation based on a top down view}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{When facing down, the number of rendered chunks stays relatively constant even when the chunk tree grows}
...
\clearpage
\section{Culling For Distance Based Selection}
\label{ch:culld}
\FloatBarrier
...
\begin{table}[h]
  \centering
  \caption[]{Settings for evaluation of culling with distance based selection}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Camera view looking over Brooklyn, Manhattan and New Jersey with distance based LOD. Chunk edges are shown to illustrate the difference in level of detail}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Culling of chunks with distance based selection}
    \label{fig:cullingd}
\end{figure}
...
\clearpage
\section{Culling For Projected Area Based Selection}
\label{ch:culla}
...
\begin{table}[h]
  \centering
  \caption[]{Settings for evaluation of culling with area based selection}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[bt]{0.4\textwidth}
        ...
        \caption{Frustum and Horizon culling}
    \end{subfigure}
    \caption{Culling of chunks with area based selection}
    \label{fig:cullinga}
\end{figure}
...
\clearpage
\section{LOD: Distance Based vs. Area Based}
\FloatBarrier
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        ymajorgrids = true,
        ylabel = {},
        symbolic x coords={Distance based LOD, Area based LOD},
        xtick = data,
        scaled y ticks = false,
      ...
      \addplot[style={chunks_color,fill=chunks_color,mark=none}]
        coordinates { (Distance based LOD, 154) (Area based LOD, 94) };

      \addplot[style={leafs_color,fill=leafs_color,mark=none}]
        coordinates { (Distance based LOD, 116) (Area based LOD, 71) };

      \addplot[style={rendered_color,fill=rendered_color,mark=none}]
        coordinates { (Distance based LOD, 53) (Area based LOD, 26) };

      \addplot[style={frame_render_time,fill=frame_render_time,mark=none}]
        coordinates { (Distance based LOD, 28) (Area based LOD, 23) };

      \addplot[style={globe_render_time,fill=globe_render_time,mark=none}]
        coordinates { (Distance based LOD, 10) (Area based LOD, 5.6) };

      \legend{Chunks, Leaf chunks, Rendered chunks, Frame time [ms], Globe render time [ms]}

    \end{axis}
\end{tikzpicture}
\caption{Comparison of Distance based LOD and Area based selection}
\label{fig:lodcomp}
\end{figure}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Comparison of level blending and no blending}
    \label{fig:blending2}
\end{figure}
...
\FloatBarrier
The impact of polar pinching on the chunk tree was evaluated using both distance based selection and area based selection. The results are presented in figure \ref{fig:pincheval}.

\begin{table}[h]
  \centering
  \caption[]{Settings for evaluation of polar pinching}
...
\todo{Vadå locations? Vi har ju bara Pole och Equator i diagrammet.}
...
\section{Camera Space Rendering}
