...
The techniques implemented to solve the issue is described under implementation \todo{ref to our implementation here} and differs somewhat from from the methods proposed by Cozzi and Ring, though the basics are derived from the underlying theories.

\subsubsection{Z-Fighting}

Another well known issue related to precision in rendering of highly detailed objects with a large range of possible distances to the camera is z-fighting. Z-fighting appears as triangles or fragments flipping between being positioned in front of or behind each other, see figure (smask) \todo{Image needed}.
