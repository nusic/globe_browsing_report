...
As chunks are tiled and rendered next to each other, it is desireable to make the borders between chunks as unnoticeable as possible. Even though the chunk meshes are generated according the requirements mentioned in the Chunk subsection above, it is not possible to guarantee a watertight edge between two adjacent chunks of different level of detail. Where adjacent chunks have different detail level, so called T-junctions till emerge. These T-junctions cause cracks between the chunks as an unwanted visual artifact. Figure \ref{fig:noskirts} below illustrates this issue.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Chunks of different LOD produce visible cracks between them.}
    \label{fig:noskirts}
\end{figure}

The easiest way to tackle this issue is not to try to remove the cracks, but instead try to hide them. The most common approach hides the cracks by simply adding an extra layer of vertices to the sides of sides of the mesh. This extra layer of vertices, which are also known as skirts, are offset down towards the ground, as illustrated in figure \ref{fig:skirts}. 
